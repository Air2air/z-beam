...
export const ScanPattern: React.FC<ScanPatternProps> = ({
  ...
  overlapRatio: initialOverlapRatio,
}) => {
  ...
  const [scanSpeed, setScanSpeed] = useState(initialScanSpeed);
  const [repRate, setRepRate] = useState(initialRepRate);
  const [overlapRatio, setOverlapRatio] = useState(initialOverlapRatio);
  
  // Reset to original values
  const handleReset = () => {
    setSpotSize(initialSpotSize);
    setScanSpeed(initialScanSpeed);
    setRepRate(initialRepRate);
    setOverlapRatio(initialOverlapRatio);
  };
  
  // Optimize parameters to achieve target overlap
  const handleOptimize = () => {
    // Keep spot size and target overlap, calculate optimal rep rate
    // Formula: overlap = (1 - (scanSpeed / repRate) / spotSize) * 100
    // Solving for repRate: repRate = scanSpeed / (spotSize * (1 - overlap/100))
    const targetOverlap = overlapRatio / 100;
    const optimalRepRate = scanSpeed / (spotSize * (1 - targetOverlap));
    setRepRate(Math.round(optimalRepRate));
  };
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-900/20 rounded-lg p-4 border border-blue-500/50">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-sm font-semibold text-blue-300">⚙️ Input Parameters</h4>
              <button
            ...
            {isEditing ? (
              <div className="space-y-3">
                {/* Editable Spot Size */}
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Spot Size (mm)</label>
                  <input
                    type="number"
                    value={spotSize}
                ...
                {/* Editable Scan Speed */}
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Scan Speed (mm/s)</label>
                  <input
                    type="number"
                    value={scanSpeed}
                    onChange={(e) => setScanSpeed(parseFloat(e.target.value) || 0)}
                    step="50"
                    min="100"
                    max="10000"
                    className="w-full px-2 py-1 bg-gray-800 border border-gray-600 rounded text-white text-sm focus:border-blue-500 focus:outline-none"
                ...
                {/* Editable Rep Rate */}
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Rep Rate (kHz)</label>
                  <input
                    type="number"
                    value={(repRate / 1000).toFixed(1)}
                    onChange={(e) => setRepRate((parseFloat(e.target.value) || 0) * 1000)}
                    step="1"
                    min="1"
                    max="200"
                    className="w-full px-2 py-1 bg-gray-800 border border-gray-600 rounded text-white text-sm focus:border-blue-500 focus:outline-none"
                ...
                {/* Editable Target Overlap */}
                <div>
                  <label className="text-xs text-gray-400 block mb-1">Target Overlap (%)</label>
                  <input
                    type="number"
                    value={overlapRatio}
                    onChange={(e) => setOverlapRatio(parseFloat(e.target.value) || 0)}
                    step="5"
                    min="0"
                    max="95"
                    className="w-full px-2 py-1 bg-gray-800 border border-gray-600 rounded text-white text-sm focus:border-blue-500 focus:outline-none"
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    onClick={handleReset}
                    className="flex-1 px-3 py-2 bg-gray-600 hover:bg-gray-700 text-white text-xs font-semibold rounded transition-colors"
                  >
                    ↺ Reset
                  </button>
                </div>
                <p className="text-xs text-gray-400 mt-2">
                  💡 Click "Optimize" to auto-calculate rep rate for target overlap
                </p>
              </div>
            ) : (
