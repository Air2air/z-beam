...
export function TagDebug() {
  ...
  if (error) {
    return (
      <div className="p-4 bg-red-50 rounded border border-red-200">
        <h3 className="font-semibold text-red-700">Tag Debug Error</h3>
  ...
  if (!data) {
    return (
      <div className="p-4 bg-yellow-50 rounded border border-yellow-200">
        <h3 className="font-semibold text-yellow-700">No Tag Data</h3>
  ...
  return (
    <div className="p-4 bg-white rounded border">
      <h3 className="text-lg font-semibold mb-3">Tag System Debug</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="bg-blue-50 p-3 rounded">
          <div className="text-2xl font-bold text-blue-700">{data.totalArticles}</div>
          <div className="text-sm text-blue-600">Total Articles</div>
        </div>
        
        <div className="bg-green-50 p-3 rounded">
          <div className="text-2xl font-bold text-green-700">{data.articlesWithAuthor}</div>
          <div className="text-sm text-green-600">Articles with Author</div>
        </div>
        
        <div className="bg-purple-50 p-3 rounded">
          <div className="text-2xl font-bold text-purple-700">{data.articlesWithFrontmatterAuthor}</div>
          <div className="text-sm text-purple-600">With Frontmatter Author</div>
        </div>
      </div>
      
      <h4 className="font-medium text-gray-700 mb-2">Sample Author Data:</h4>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.tags && item.tags.length > 0 ? 
                        item.tags.map((tag, tagIdx) => (
                          <span key={tagIdx} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                            {tag}
                          </span>
                        ))
