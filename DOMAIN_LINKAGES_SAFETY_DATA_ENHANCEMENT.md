...
```yaml
domain_linkages:
  produces_compounds:  # ⚠️ MUST be ordered by severity: severe → high → moderate → low
    - id: string                          # Compound slug (required)
      ...
      severity: enum                      # Overall severity (required) - USED FOR SORTING
...
# severity: Overall hazard severity (USED FOR ORDERING)
- severe        # Life-threatening, permanent damage risk (DISPLAY FIRST)
- high          # Severe health effects possible
- moderate      # Significant irritation, temporary effects
- low           # Minor irritation, no long-term effects (DISPLAY LAST)
...
def validate_migration(file_path):
    ...
    # Check: Compounds are sorted by severity
    severity_order = {'severe': 0, 'high': 1, 'moderate': 2, 'low': 3}
    for i in range(len(produces) - 1):
        current_severity = severity_order.get(produces[i].get('severity', 'low'), 3)
        next_severity = severity_order.get(produces[i+1].get('severity', 'low'), 3)
        if current_severity > next_severity:
            issues.append(
                f"⚠️ Incorrect sort order: {produces[i]['title']} ({produces[i]['severity']}) "
                f"before {produces[i+1]['title']} ({produces[i+1]['severity']})"
            )
...
def enhance_domain_linkages(contaminant_data):
    ...
    # Sort compounds by severity (most severe first)
    severity_order = {'severe': 0, 'high': 1, 'moderate': 2, 'low': 3}
    produces_compounds.sort(
...
# Ordering rules
ordering:
  - produces_compounds must be sorted by severity (severe → high → moderate → low)
...
**HazardousCompoundsGrid / Enhanced Domain Linkage Display**:
- ✅ **USE ON**: Contamination pages only
  - `/contaminants/**/*-contamination` pages
...
- ❌ **DO NOT USE ON**: Materials pages, Settings pages
  - Materials pages use standard MaterialCard grids (not domain-linkage variant)
  - Settings pages have their own specialized components
  - Different data structure and display requirements

**Rationale**:
- Only contaminants produce hazardous compounds during removal
- Materials don't generate compounds (they're substrates, not contaminants)
- Settings are machine parameters, unrelated to compound safety data
...
### Unified Table Component (Contamination Pages Only)
```tsx
// ✅ CORRECT: On contamination pages
...
// ❌ INCORRECT: Do not use on materials or settings pages
// These pages have different data structures
```
...
- **Default sort**: Severity (severe → high → moderate → low)
- **Secondary sorts**: Concentration, alphabetical
...
### Contamination Pages
**Data Structure**: `domain_linkages.produces_compounds`
- Enhanced with concentration ranges, exposure limits
- Displayed using HazardousCompoundsGrid component
- Shows compounds generated during contaminant laser removal
- Includes safety warnings and PPE requirements

### Materials Pages
**Data Structure**: `domain_linkages.compatible_materials` or `related_materials`
- Uses standard MaterialCard component (not domain-linkage variant)
- No compound data (materials don't produce compounds)
- Displays material properties, laser parameters
- Shows contaminant compatibility

### Settings Pages
**Data Structure**: Machine settings, laser parameters
- Specialized settings components
- No domain linkages
- No compound safety data
...
1. **Should material linkages also be enhanced?**
   ...
   - Materials don't produce compounds (only contaminants do)
