...
- **Extend and enhance** existing functionality rather than duplicate
- **Refactor and consolidate** similar patterns into shared utilities
- **Remove dead code** and unused components during optimization
- **Document why** new components are necessary when existing ones can't be extended

##### ❌ DON'T:
- Create new components without first checking for existing similar functionality
- Duplicate logic or UI patterns across multiple files
- Add new dependencies when existing ones can be optimized
- Create "wrapper" components that don't add meaningful value
- Assume new files are needed without thorough analysis

#### Process for New Components:
1. **Search existing codebase** for similar functionality
2. **Analyze extension possibilities** of existing components
3. **Document why existing solutions are insufficient**
...
```typescript
// Extend SmartTagList instead of creating new badge component
<SmartTagList tags={tags} variant="compact" linkable={false} />
```

**❌ INCORRECT - Create Duplicate:**
```typescript
// DON'T create a new BadgeList component when SmartTagList exists
