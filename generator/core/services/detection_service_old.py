...
class DetectionService(IDetectionService):
    ...
    def __init__(self, api_client: IAPIClient, prompt_repository: IPromptRepository):
        ...
        self._natural_voice_prompt_variations = [
            "natural_voice_enhanced",  # New enhanced prompt as default
    ...
    def detect_human_likelihood(
        ...
        """Detect Natural Voice characteristics in content using optimal prompt variation."""
        section_name = context.get_variable("section_name", "Unknown")
        optimal_prompt = self._optimizer.get_optimal_prompt(
            "natural_voice",
            iteration,
        ...
        logger.info(
            f"Using Natural Voice detection prompt variation: {optimal_prompt} (iteration {iteration})"
        )
