...
class GlobalConfigManager:
    ...
    def initialize(cls, user_config: Dict[str, Any], provider_models: Dict[str, Any] = None) -> 'GlobalConfigManager':
        ...
        # Create temperature config from user settings
        instance._temperature_config = TemperatureConfig(
            content_temp=user_config.get("content_temp", 0.6),
            detection_temp=user_config.get("detection_temp", 0.3),
            improvement_temp=user_config.get("improvement_temp", 0.7),
            summary_temp=user_config.get("summary_temp", 0.4),
            metadata_temp=user_config.get("metadata_temp", 0.2),
        )
