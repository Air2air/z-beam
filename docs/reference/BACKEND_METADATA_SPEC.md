# Backend Metadata Field Naming Specification

**Version:** 5.0.0  
**Last Updated:** December 26, 2025  
**Purpose:** Eliminate frontend mapping requirements by using exact field names

⚠️ **TERMINOLOGY**: This document uses "metadata" as the canonical term. References to "frontmatter" in YAML context refer to the YAML header block, but when accessing data programmatically, always use `article.metadata`, never `article.frontmatter`.
...
author:
  ...
  expertiseAreas: string[]            # Array of expertise areas (plural for arrays)
  credentialsList: string[]          # Array of credentials/qualifications (plural)
  affiliation:
...
# Flattened expertise (wrong)
expertiseAreas: "Laser Processing, Surface Engineering"  # Should be array

# Using 'expertise' instead of 'expertiseAreas' (inconsistent)
expertise: []  # Should be expertiseAreas for clarity
```
...
Before generating metadata YAML, verify:
...
- [ ] All arrays use plural names (`expertiseAreas`, not `expertise`)
