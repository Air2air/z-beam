...
| `custom_label_3` | **Safety rating** | `high-hazard` / `moderate-hazard` / `low-hazard` / `severe-hazard` |
| `custom_label_4` | **Ventilation requirement** | `industrial-ventilation` / `enhanced-ventilation` / `standard-ventilation` |

### Safety Custom Labels (custom_label_3 & custom_label_4)

**Purpose**: Enable safety-based filtering and campaign segmentation in Google Shopping.

#### custom_label_3: Safety Rating

**Values** (based on aggregated fire/toxic/visibility risks):
- `severe-hazard`: Requires professional operator, highest safety protocols
  - Example: Lead paint, chromium compounds, toxic metallic coatings
  - Criteria: toxic_gas_risk = "severe" OR fire_explosion_risk = "critical"
- `high-hazard`: Enhanced safety equipment and training required
  - Example: Zinc coatings, industrial solvents, asbestos-containing materials
  - Criteria: Any risk rated "high" or multiple "moderate" risks
- `moderate-hazard`: Standard safety protocols with enhanced PPE
  - Example: Aluminum oxide, organic coatings, rust/oxidation
  - Criteria: Any risk rated "moderate"
- `low-hazard`: Basic safety equipment sufficient
  - Example: Clean metals (aluminum, stainless steel), ceramics, glass
  - Criteria: All risks rated "low" or "minimal"
...
**Values** (based on ventilation_requirements.minimum_air_changes_per_hour):
- `specialized-extraction`: Local exhaust capture system required
  - Criteria: minimum_air_changes_per_hour >= 15 OR local_exhaust = true
  - Example: Toxic metals (lead, cadmium), hazardous chemicals
- `industrial-ventilation`: High-capacity general ventilation
  - Criteria: minimum_air_changes_per_hour 10-14
  - Example: High-particulate materials, chemical residues
- `enhanced-ventilation`: Enhanced general ventilation
  - Criteria: minimum_air_changes_per_hour 6-9
  - Example: Moderate oxidation, organic residues
- `standard-ventilation`: Standard workplace ventilation
  - Criteria: minimum_air_changes_per_hour < 6 OR not specified
  - Example: Clean metals, ceramics, glass

**Campaign Segmentation Use Cases**:
1. **Safety-Focused Campaigns**: Target "low-hazard" + "standard-ventilation" for DIY/small business audience
2. **Industrial Campaigns**: Target "high-hazard" + "industrial-ventilation" for large facilities with existing safety infrastructure
3. **Regulatory Compliance**: Highlight "severe-hazard" materials with required certifications and professional-only service
4. **Budget Segmentation**: Low-hazard materials = shorter estimated hours, better for cost-conscious buyers

**Feed Example with Safety Labels**:
```xml
<item>
  <g:id>lead-paint-laser-cleaning-professionalCleaning</g:id>
  <g:title>Professional Laser Cleaning for Lead Paint</g:title>
  <g:custom_label_0>Professional Laser Cleaning</g:custom_label_0>
  <g:custom_label_1>Hourly Rate: $390/hour</g:custom_label_1>
  <g:custom_label_2>Est. Range: $780-$2340</g:custom_label_2>
...
- Description extraction priority (description → micro → generated)
