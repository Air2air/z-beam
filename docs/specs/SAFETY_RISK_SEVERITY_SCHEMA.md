...
safety_data:
  ...
  # FORMAT 1: Simple string (materials)
  ...
  # FORMAT 2: Nested object with details (contaminants)
  fire_explosion_risk:
    severity: "low"                    # Severity level (required)
    description: "Minimal fire risk with standard precautions"
    mitigation: "Standard fire safety precautions, extinguisher available"
  
  toxic_gas_risk:
    severity: "moderate"
    primary_hazards:                   # Array of detected compounds
      - compound: "Carbon Monoxide"
        concentration_mg_m3: 5.0
        hazard_class: "toxic"
    description: "Multiple toxic compounds detected"
    mitigation: "N95 respirator required, enhanced ventilation"
  
  visibility_hazard:
    severity: "moderate"
    description: "Moderate visibility reduction (40-60%)"
    source: "Respirable fraction: 0.80"
    mitigation: "Ensure clear sight lines, use source extraction"
  ...
  ppe_requirements:
    ...
    rationale: "Protection justification (contaminants only)"
...
// - Simple string: safetyData.fire_explosion_risk = "moderate"
// - Nested object: safetyData.fire_explosion_risk = { severity: "moderate", ... }

<RiskCard
  icon={Flame}
  label="Fire/Explosion Risk"
  severity={
    typeof safetyData.fire_explosion_risk === 'string'
...
- **Risk levels**: 
  - Simple format: Must be string from `[critical, high, moderate, medium, low, none]`
  - Nested format: Object with `severity` field (string from above) + optional `description`, `mitigation`, `primary_hazards`, `source`
...
- **Materials**: Use simple string format for cleaner frontmatter
- **Contaminants**: Use nested object format for detailed safety data
- **Components**: Automatically detect and handle both formats (SafetyOverview, SafetyDataPanel)
...
All frontend components have been verified for schema compliance:

**Core Utility**:
- ✅ `getRiskColor()` - [app/utils/layoutHelpers.ts](../../app/utils/layoutHelpers.ts)
  - Supports all severity levels: critical, high, moderate, medium, low, none
  - Case-insensitive matching
  - Tested: 8/8 tests passing

**Display Components**:
- ✅ `RiskCard` - [app/components/RiskCard/RiskCard.tsx](../../app/components/RiskCard/RiskCard.tsx)
  - Displays risk cards with color-coded severity
  - Schema reference in documentation
  
- ✅ `InfoCard` - [app/components/InfoCard/InfoCard.tsx](../../app/components/InfoCard/InfoCard.tsx)
  - Displays PPE, ventilation, particulate data
  - Consistent styling with RiskCard
  
- ✅ `SafetyDataPanel` - [app/components/SafetyDataPanel/SafetyDataPanel.tsx](../../app/components/SafetyDataPanel/SafetyDataPanel.tsx)
  - Materials safety display with unified grid
  - All schema fields mapped and documented
  
- ✅ `SafetyOverview` - [app/components/Contaminants/SafetyOverview.tsx](../../app/components/Contaminants/SafetyOverview.tsx)
  - Contaminants safety display with unified grid
  - Identical implementation to SafetyDataPanel

### Field Mapping Verification

```typescript
// Risk Assessment - Direct mapping
safetyData.fire_explosion_risk     → RiskCard (severity prop)
safetyData.toxic_gas_risk          → RiskCard (severity prop)
safetyData.visibility_hazard       → RiskCard (severity prop)

// PPE Requirements - InfoCard data array
safetyData.ppe_requirements.respiratory      → { label, value }
safetyData.ppe_requirements.eye_protection   → { label, value }
safetyData.ppe_requirements.skin_protection  → { label, value }

// Ventilation Requirements - InfoCard data array
safetyData.ventilation_requirements.minimum_air_changes_per_hour → { label, value }
safetyData.ventilation_requirements.exhaust_velocity_m_s         → { label, value }
safetyData.ventilation_requirements.filtration_type              → { label, value }

// Particulate Generation - InfoCard data array
safetyData.particulate_generation.respirable_fraction  → { label, value: "65%" }
safetyData.particulate_generation.size_range_um        → { label, value: "0.5-10 μm" }
```

### Test Coverage

**Unit Tests**: [tests/utils/layoutHelpers.test.ts](../../tests/utils/layoutHelpers.test.ts)
- ✅ All severity levels tested (critical, high, moderate, medium, low, none)
- ✅ Case-insensitive matching verified
- ✅ Undefined handling tested
- **Result**: 8/8 tests passing

### Display Formatting

- **Severity**: Capitalized on display (CSS `capitalize` class)
- **Respirable Fraction**: Converted to percentage (value × 100 + "%")
- **Size Range**: Formatted with units ("X-Y μm")
- **Exhaust Velocity**: Formatted with units ("X m/s")
- **Risk Colors**: Applied via `getRiskColor()` utility

---

## Changelog

**December 20, 2025 - v1.2**
- ✅ Added dual format support: simple strings (materials) and nested objects (contaminants)
- ✅ Updated SafetyOverview to handle both risk field formats automatically
- ✅ Documented type checking pattern: `typeof risk === 'string' ? risk : risk.severity`
- ✅ Added contaminants-specific nested structure with description, mitigation, primary_hazards

**December 20, 2025 - v1.1**
- ✅ Frontend implementation completed and verified
- ✅ All components updated for schema compliance
- ✅ Test coverage added for all severity levels
- ✅ Field mapping documentation added
- ✅ TypeScript compliance verified (0 errors)

**December 20, 2025 - v1.0**
